...
#define KONG_LOG_DUBUG(logger) KONG_LOG_LEVEL(logger, kong::LogLevel::DEBUG)
...
#define KONG_LOG_FMT_LEVEL(logger, level, fmt, ...) \
    if(logger->getLevel() <= kong::LogLevel::ERROR) \
        kong::LogEventWrap(kong::LogEvent::ptr(new kong::LogEvent(logger, level, \
        __FILE__, __LINE__, 0, kong::GetThreadId(), kong::GetFiberId(), \
        time(0), kong::Thread::GetName()))).getSS().getEvent().format(fmt, __VA_ARGS__)
